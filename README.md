...
This repository hosts the publicly accessible brand logo asset used by the BIMI standard to display the 4th and Bailey logo directly inside supporting email inboxes, helping recipients visually identify and trust email sent from our domain.
...
| File | Description |
|---|---|
| `logo.svg` | 4th and Bailey brand logo in SVG Tiny P/S format, served via GitHub Pages for BIMI logo display |
...
BIMI requires a fully enforced DMARC policy. The domain `4thandbailey.com` has DMARC, DKIM, and SPF fully implemented and enforced across all sending sources.

---

## Validation

Validate the BIMI setup at:
...
- [ ] Obtain a **Verified Mark Certificate (VMC)** to enable the Gmail blue verified checkmark
